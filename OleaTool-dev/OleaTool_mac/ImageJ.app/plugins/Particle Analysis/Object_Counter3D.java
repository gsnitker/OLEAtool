...
2006/05/31: Version 1.3 (Jonathan Jackson, j.jackson at ucl.ac.uk)
...
2006/06/14: Version 1.4 (Jonathan Jackson, j.jackson at ucl.ac.uk)
 
 * Plugin remembers user selected options
 * Fixed bug causing particles to wrap-around image boarders

2007/06/18: Version 1.5 (Jonathan Jackson, jjackson at familyjackson dot net )
 
 * Measurements now spatially calibrated 
 * Affected values:
 *    Volume
 *    Centre of Mass / Centroid
 * 'Surface' value is now expressed as the fraction of voxels on the object ace
 */
...
public class Object_Counter3D implements PlugIn, AdjustmentListener, TextListener, Runnable {
    ...
    void analyze() {
        ...
        double voxelSize = ((pixelDepth * pixelWidth)+ (pixelDepth * pixelHeight)+(pixelHeight*pixelDepth))/3;
        ...
        String[] head={"Volume","Surface#:Volume#","Intensity","Centre X","Centre Y","Centre Z","Centre int X","Centre int Y","Centre int Z"};
...
//                rt.addValue(1,Paramarray[i][1]*voxArea);
        ...
        if (showCentres){ // Create 'Centres' image
            ...
            for (i=0;i<nParticles;i++){
                ip=stackCentres.getProcessor((int)Math.round(rt.getValue(5,i)/pixelDepth+zOrigin));
                ip.setValue(i+1);
        ...
        if (showCentresInt){ // Create 'Intensity weighted Centres' image
            ...
            for (i=0;i<nParticles;i++){
                ip=stackCentresInt.getProcessor((int)Math.round(rt.getValue(8,i)/pixelDepth+zOrigin));
                ip.setValue(i+1);
                ip.setLineWidth(DotSize);
