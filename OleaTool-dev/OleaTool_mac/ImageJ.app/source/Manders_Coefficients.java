...
if(IJ.isResultsWindow()==false)  headingsSet = false;
	...
	gd.addChoice("Use ROI", chooseROI, chooseROI[indexRoi]);
		gd.addCheckbox("Current_Slice_only", useSlice);
	gd.addCheckbox("Display_Frequency Scatterplot ", showFreqPlot);
        	gd.addCheckbox("Display_Colour Scatterplot ", showColourPlot);
	gd.addCheckbox("Display_Counts ", displayCounts);
        	gd.addCheckbox("Use_threshold",threshold);
	gd.addCheckbox("Exclude_zero-zero_pixels", exclude);
	gd.showDialog();
        	if (gd.wasCanceled())
	            return false;
