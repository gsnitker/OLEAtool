...
public class Grid_Confluency implements PlugInFilter {
	...
	void drawPoints(ImageProcessor ip) {
		...
		int two = lineWidth*2;
		int radius = 3;
		ImageProcessor ip2 = new ByteProcessor (radius, radius);
		int [] row1 = new int [radius*radius];
			...
			for(int v=0; v<linesH; v++) {
				...
				for (int r=0; r<radius; r++)
					...
					for(int c =0;c<radius; c++)
						...
						{
						p=ip.getPixel(x+c, y-r);
						ip2.putPixel(c,r,p);
						}
					}
				ImagePlus imp2 = new ImagePlus("imp2", ip2);
				stat1 = imp2.getStatistics();
				meanP = stat1.mean;
				stddevP = stat1.STD_DEV;

				//ip2 = new ByteProcessor(row1);
