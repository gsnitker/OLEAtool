...
yMin=0; yFmin=0;
		...
		for (int u = 0; u<y.length; u++)
			{
			y[u] = ((yF[(2*u)+1]) / (yMax) ) / ((yF[2*u])/(yFmax));

			//IJ.write(""+y[u] +" / "+ yF[u]  +" / "+ y[u]);
		...
		//fit curve
			...
			IJ.log("p[0]+p[1]*Math.log(x-p[2]):  "+ df2.format(p[0])+";  "+ df2.format(p[1]) +";  "+df2.format(p[2]));
			double tmp=0;
			...
			for (int z=0; z<x2.length; z++)
				...
				fit[z]=(float)(p[0]+p[1]*Math.log(tmp));
				}
