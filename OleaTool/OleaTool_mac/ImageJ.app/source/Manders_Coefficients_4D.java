...
public class Manders_Coefficients_4D implements PlugIn
	 {
	static boolean headingsSet;
	private static int index1;
	private static int index2;
	private ImageStack img1, img2;
	private int zSteps = 10;
...
public void run(String arg) 
	{
	if (showDialog())	
	            correlate(img1, img2, zSteps);
	}
    	
public boolean showDialog() {

	if(IJ.isResultsWindow()==false)  headingsSet = false;
	...
	gd.addNumericField("Z-steps", zSteps,0);
...
//	if (imp1.getType()!=imp1.GRAY8&&imp1.getType()!=imp1.GRAY16&&imp2.getType()!=imp2.GRAY16 &&imp2.getType()!=imp1.GRAY8) 
	if (imp1.getType()!=imp1.GRAY8&&imp1.getType()!=imp1.GRAY16) 
			...
			int firstSlice=1;
			int lastSlice = imp1.getStackSize();
			
			for (int cs=firstSlice; cs<=lastSlice; cs++)
				{
...
public void correlate(ImageStack img1, ImageStack img2, int zSteps) 
	{
	
	int nSlices = imp1.getStackSize();
	int endT = nSlices/zSteps;
	int nslices=0;

   if((nSlices % zSteps)==0) {
            String sError = "Wrong z-step: ";
	}
...
for(int t=0; t<endT; t++){
	...
	int startSlice =1 +(t*zSteps);
	int endSlice = zSteps + (t*zSteps);

	//IJ.write(startSlice + " - " + endSlice);
...
//	new ImagePlus("green", ip2).show();
			...
			{mask = 1;
				...
				if (mask!=0)
					...
					if (((ch1>=ch1threshmin)&&(ch2>=ch2threshmin))||!threshold) 
						include = true;
					else include = false;	
...
//	      	IJ.write(Headings);
	...
	IJ.write(fileName  + startSlice +"-" + endSlice 
